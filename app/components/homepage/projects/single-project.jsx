...
const SingleProject = ({ project }) => {
  const { name, description, tools, code, demo, image, keyFeatures } = project;
  ...
  return (
    <div className="group w-full h-fit flex flex-col items-center justify-center relative cursor-text overflow-hidden px-3 md:px-8 py-[1.4rem] bg-[linear-gradient(90deg,#281e57_0%,#201435_100%)] shadow-2xl rounded-lg border border-[#1a1443]">
      ...
      <div className="flex flex-col items-center justify-between w-full h-full">
        ...
        <div className="relative z-30 flex items-center justify-between w-full">
          <Link
            href={demo}
            target="_blank"
            className="relative z-40 flex justify-center items-center w-10 h-10 rounded-full border-2 border-[#EFF3F4] text-[#EFF3F4] transition-all duration-300 hover:bg-[#231d4b] hover:text-violet-600 hover:border-[#0F0C41] hover:scale-110 decoration-clone cursor-pointer no-underline delay-[0.3s]"
          >
            <FaPlay />
          </Link>
          ...
          <Link
            href={code}
            target="_blank"
            className="relative z-40 flex justify-center items-center w-10 h-10 rounded-full border-2 border-[#EFF3F4] text-[#EFF3F4] transition-all duration-300 hover:bg-[#231d4b] hover:text-violet-600 hover:border-[#0F0C41] hover:scale-110 cursor-pointer no-underline  delay-[0.3s] group-hover:translate-x-[-140px]"
          >
            <FaCode />
          </Link>
        </div>
      </div>
      {/* <p className="absolute pointer-events-none w-[90%] md:w-[85%] md:min-h-[150px] translate-x-[-110%] transition-transform duration-[0.9s] p-6 leading-[110%] rounded-[0_20px_20px_0] left-0 top-0 bg-[#0f0b24]  text-[#EFF3F4] translate-y-[25%] md:translate-y-[50%] group-hover:translate-x-[-2%] text-xs md:text-sm">
        {description}
      </p> */}
      <div className="absolute pointer-events-none w-[90%] md:w-[80%] md:min-h-[150px] translate-x-[-110%] transition-transform duration-[0.9s] p-6 leading-[110%] rounded-[0_20px_20px_0] left-0 top-0 bg-[#0f0b24] text-[#EFF3F4] translate-y-[25%] md:translate-y-[50%] group-hover:translate-x-[-2%] text-xs md:text-sm">
        <p className="mb-3">{description}</p>

        {keyFeatures?.length > 0 && (
          <ul className="space-y-1">
            {keyFeatures.slice(0, 4).map((feature, index) => (
              <li key={index} className="text-[11px] md:text-xs">
                • {feature}
              </li>
            ))}
          </ul>
      ...
      <div className="pointer-events-none group-hover:translate-x-0 absolute w-[140px] text-[0.8rem] flex justify-center gap-2 flex-col translate-x-full transition-transform duration-[0.5s] delay-[0.3s] p-[0.825rem] rounded-[10px_0_0_10px] right-0 bottom-4 bg-[#0f0b24] text-[#EFF3F4]">
        {tools.map((tag, id) => (
